...
from app1.models import NutritionModel,Nutrition_item,diet,diet_detail
...
def check_nutri(request):
    ...
    data={'gen':gen,'age':age,'height':height,'weight':weight,'bodyfat':bodyfat,'bodyphy':bodyphy,
    ...
    if activity=='Sedentary':
        reqs={'energy':(2320/60)*weight,'fat':(25/60)*weight,'pro':pro*weight}
    elif activity=='Lightly Active':
        reqs={'energy':(2520/60)*weight,'fat':(27.5/60)*weight,'pro':pro*weight}
    elif activity=='Moderately Active':
        reqs={'energy':(2730/60)*weight,'fat':(30/60)*weight,'pro':pro*weight}
    elif activity=='Intence Weight training/sports'or "Very Active" or "Extra Active":
        reqs={'energy':(3490/60)*weight,'fat':(40/60)*weight,'pro':pro*weight}
    

    print('reqs:',reqs)
#uncomment below lines to add more entries into table using csv files
            
    # csv_fp = open(f'z_diet_plas.csv', 'r')
    # reader = csv.reader(csv_fp)
    # read = pd.read_csv("z_items.csv",error_bad_lines=False)
    # fields = []
    # rows = []
    # with open('z_diet_plas.csv', newline='') as csvfile:
    #     data = csv.reader(csvfile, delimiter=',')
    # # Following command skips the first row of the CSV file:
    #     fields = next(data)
    #     #fields = next(data)
    #     #data = [float(p) for p in data[1:65]]
    #     for row in data:
    #         #print(row)
    #         li=row
    #         a=int(li[0])
    ...
    #         if de:
    #             de[0].Kcal+=1
    #             diet_detail.objects.filter(diet_sr=a).update(Kcal=F('Kcal') + de[0].Kcal*d,Protein=F('Protein') + de[0].Protein*d,Fat=F('Fat') + de[0].Fat*d,Carbs=F('Carbs') + de[0].Carbs*d)
    #             #print('done',e)
    #         else:
    #             print(e)
    #         diet.objects.create(Product=e,copy=d,diet_sr=a,diet_day=b,diet_time=c,type=f)
    #     diet_detail.objects.update(Kcal=F('Kcal') /7,Protein=F('Protein') / 7,Fat=F('Fat') / 7,Carbs=F('Carbs') / 7)
    ...
    #         Nutrition_item.objects.create(Product=a,Kcal=b,Protein=c,Fat=d,Carbs=e,T_sat_fat=f,T_umo_fat=g,T_upo_fat=h,choles=i,sodium=j,T_fi=k,Vit_A_Re=l,Vit_A_IU=m,alpha_toco=n,ascor=o,thiami=p,iaci=q,Vit_B6=r,folaci=s,Vit_B12=t,Potassium=u,Calcium=v,Phosphorus=w,Magnesium=x,Iron=y,Zinc=z,Pantothenic_acid=aa,Copper=ab,Manganese=ac,Ash=ad,Water=ae,energy_kJ=af,Caprylic_Acid=ag,Capric_Acid=ah,Lauric_Acid=ai,Myristic_Acid=aj,Palmitic_Acid=ak,Palmitoleic_Acid=al,Stearic_Acid=am,Oleic_Acid=an,Linoleic_acid=ao ,Linolenic_acid=ap,Gadoleic_acid=aq,Docosenoic_acid=ar,Phytosterols=asa,Histidine=at,Isoleucine=au,Leucine=av,Lysine=aw,Methionine=ax,Cystine=ay,Methionine_Cystine=az,Phenylalanine=ca,Tyrosine=cb,Phenylalanine_Tyrosine=cc,Threonine=cd,Tryptophan=ce,Valine=cf,Arginine=cg,Alanine=ch,Aspartic_acid=ci,Glutamic_acid=cj,Glycine=ck,Proline=cl,Serine=cm)
    #     #diet.objects.create(Product=c,copy=b,diet_sr=a,type=d)
            #print(s)
    ...
    if res:
        ...
        rel=100
    print(rel)
    ret=diet_detail.objects.filter(Protein=rel,disease__contains=diseases,type=food_habbit,regio__contains=food_preference)
...
def update_view(request, pk):
    ...
    return render(request, "diet_plan/update.html", context)
...
def updt_diet1(request,id):
    with open('diet_updt.csv', newline='') as csvfile:
         ...
         for row in data:
             ...
             print(d)
    try:
        diet1=diet.objects.filter(diet_sr=a)
        ret=diet_detail.objects.filter(diet_sr=a)
    ...
    #print(ret[0].diet_sr)
    #diet2=list(diet1)
    return render(request,"diet_plan/show.html",{'diet1':diet1,'reqs':reqs,'give':ret[0]})
